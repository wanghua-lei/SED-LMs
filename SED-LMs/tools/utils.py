...
def decode_output(predicted_output, ref_captions, file_names,
    ...
    if beam_size == 0:
        logging = logger.add(str(log_output_dir) + '/beam_captions_{}ep_top9.txt'.format(epoch),
                             format='{message}', level='INFO',
                             filter=lambda record: record['extra']['indent'] == 4)
        caption_logger = logger.bind(indent=4)
        caption_logger.info('Captions start')
        caption_logger.info('Top 0.9 sampling search:')
    elif beam_size == 1:
        logging = logger.add(str(log_output_dir) + '/captions_{}ep_greedy.txt'.format(epoch),
                             format='{message}', level='INFO',
        ...
        caption_logger.info('Captions start')
        caption_logger.info('Beam search:{}'.format(beam_size))
    elif beam_size == 3:
        logging = logger.add(str(log_output_dir) + '/beam_captions_{}ep_{}bsize.txt'.format(epoch, beam_size),
                             format='{message}', level='INFO',
                             filter=lambda record: record['extra']['indent'] == 2)
