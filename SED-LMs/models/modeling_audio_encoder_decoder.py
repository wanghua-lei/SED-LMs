...
class BertWithMultiCrossAttention(BertLMHeadModel):
    def __init__(self, config, num_cross_attention_layers=3):
        super().__init__(config)
        self.config  = config
        self.cross_attentions = torch.nn.ModuleList([
            BertAttention(config) for _ in range(num_cross_attention_layers)
        ])
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, 
        ...
        sequence_output = outputs[0] #torch.Size([1, 1, 30522])
...
class AudioEncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        if encoder is None:
            encoder = AudioEncoderModel(config.encoder) #beats
