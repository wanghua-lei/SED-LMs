...
class BartCaptionModel(nn.Module):
    ...
    def forward_encoder(self, audios):
        outputs = self.encoder(audios) #(1,token,1024)
    ...
    def forward_decoder(self, text, encoder_outputs):
        ...
        #text to embeding
        #text_embeding = self.decoder.model.decoder.embed_tokens(input_ids) #torch.Size([batch, token_len, 768])
        ...
        loss = self.loss_fct(lm_logits.view(-1, self.tokenizer.vocab_size), decoder_targets.view(-1)) #torch.Size([6, 50265]) torch.Size([6])
    ...
    def generate(self,
        ...
        audio_embs = self.forward_encoder(samples)  #batch x token x 768

        # Encoder pass #batch x token x 768
        encoder_outputs = self.decoder.model.encoder(
            input_ids=None,
            attention_mask=None,
            head_mask=None,
        ...
        input_ids[:, 0] = self.decoder.config.decoder_start_token_id # batch x 1
...
if __name__ == '__main__':
    ...
    text = ["cat heard from 2.84 to 5.79 seconds dog heard from 6.52 to 8.49 seconds"]
