...
class Cnn10(nn.Module):
    def __init__(self, config):
        ...
        self.is_spec_augment = config.spec_augment
...
class Cnn14(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.gru = nn.GRU(2048, 512*2,  bidirectional=True, batch_first=True)
    ...
    def forward(self, input):
        ...
        x = self.audio_feats_extractor(input)   #(torch.Size([16, 1, 1001, 64]))
        ...
        x = F.dropout(x, p=0.2, training=self.training) #（batch_size, 2048, 15, 1)

        x = torch.mean(x, dim=3)  # （batch_size x 2048 x 15)

        #unsampled to (batch,150,2048)
        x = F.interpolate(x, scale_factor = 8, mode='nearest')
        #x = F.interpolate(x, 501, mode="linear", align_corners=False)
        x = x.permute(0, 2, 1)
...
class ResNet38(nn.Module):
    def __init__(self, spec_augment):

        super(ResNet38, self).__init__()
        self.bn0 = nn.BatchNorm2d(64)

        self.is_spec_augment = spec_augment
        if self.is_spec_augment:
            self.spec_augmenter = SpecAugmentation(time_drop_width=64,
                                                   time_stripes_num=2,
                                                   freq_drop_width=8,
